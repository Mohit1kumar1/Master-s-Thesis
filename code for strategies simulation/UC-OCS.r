...
##              222    223    224    227    226
##        RND   MAX-F  MSD-1  MSD-2  UC-OCS UC-OCS-2
## ------------------------------------------------- 
## SE-IL  GEV   <-     <-     <-     <-     <-
## CR-IL  RND   MAX    MAX    MAX    UC-OCS MAX
## CR-F1  RND   MAX    MAX    MAX    UC-OCS MAX  
## SE-C1  GEV   <-      |      |     |      |
## CR-C1  RND   <-     MSD    MSD    UC-OCS UC-OCS 
## SE-C2  GEV   <-     <-      |     |      <-
## CR-C2  RND   <-     <-     MSD    UC-OCS <- 
## SE-C3  GEV   <-     <-     <-     <-     <-
...
NREP <- 100 # replications for the simulation
NRUN <- 20  # how many sets of crosses are evaluated
...
eff.file <- "data/effect.file.eff"

st.read.marker.data ("marker.data.mpo",format="m",data.set="PP")
st.read.map         ("map.file.txt",m.stretch=1,skip=1, format="mcp",data.set="PP")
...
# Cross selected parental lines                        # CR-IL: MAX
...
crs  <- gs.cross.info(data.set="F1",sortby ="ma")      # CR-F1: MAX
...
write.table (e,"data/c002-126-e002.dta")  #  1000 reps
