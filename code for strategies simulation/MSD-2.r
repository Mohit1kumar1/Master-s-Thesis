...
##              122    123    124    
##        RND   MAX-F  MSD-1  MSD-2  
## --------------------------------- 
## SE-IL  GEV   <-     <-     <-     
## CR-IL  RND   MAX    MAX    MAX    
## CR-F1  RND   MAX    MAX    MAX     
## SE-C1  GEV   <-      |      |     
## CR-C1  RND   <-     MSD    MSD     
## SE-C2  GEV   <-     <-      |     
## CR-C2  RND   <-     <-     MSD     
...
llibrary ("ggplot2"); library ("gridExtra")
...
genotype.population("default")
evaluate.population("default", "yld")

# Select the best 144 P lines / GEGV

copy.population("PA","default")
genotype.population("PA")
evaluate.population("PA", "yld")
population.sort("PA", decreasing=TRUE) 
population.divide("Psel", "PA", 144)                # SE-L: GEGV
...
C <- 2; {
    ...
    crs  <- gs.cross.info(data.set="SYN",sortby ="es")      # CR-C1: MSD
    d.sorted <- data.frame(OTU1     =crs$P1No,
                           OTU2     =crs$P2No,
...
eval <- c("PP","Psel","SYN1","SYN2","SYN3","DH")
...
########################################################################
#  write.table (e,"data/c002-124-e001.dta")  #   200 reps
  write.table (e,"data/c002-124-e002.dta")  #   1000 reps